...
Talks directly to ``https://api.openai.com/v1/chat/completions`` over httpx so
we don't have to pull in the full ``openai`` SDK. The wire format is the
canonical OpenAI chat-completions shape; OpenRouter, Groq, etc. are all clones
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=DEFAULT_TIMEOUT_S) as client:
            ...
            if r.status_code >= 400:
                # Surface OpenAI's error body so the user sees the cause
                # (insufficient quota, invalid model id, rate limit, etc.).
                detail = r.text[:500]
